# AWS-microservices-architecture

[Microservices and cloud based architecture](../blob/master/img/microservices_and_cloud_based_architecture.pdf)
[Microservices on AWS](../blob/master/MS-AWS.md)

## A simple architecture

![A simple architecture on AWS](../blob/master/img/ms-simple.svg)

## A microservice architecture on AWS

![A microservice architecture on AWS](../blob/master/img/ms-architecture-aws.svg)

## A geo distributed architecture 

![A geo distributed architecture](../blob/master/img/ms-geo-distributed.svg)
